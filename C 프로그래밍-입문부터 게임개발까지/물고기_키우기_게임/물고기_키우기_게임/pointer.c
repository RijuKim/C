...
//물이 다 증발하기 전에 어항에 물을 줘 물고기를 살려야 한다
...
int main(void)
{
	...
	long prevElasedTime = 0; //직전 경과 시간
	...
	while (1) {
		...
		prevElasedTime = totalElapsedTime - prevElasedTime;
		printf("최근 경과 시간: %ld 초\n", prevElasedTime);

		//어항의 물을 감소(증발)
		decreaseWater(prevElasedTime);
		...
		if (checkFishAlive() == 0) {
			...
			//최소 한마리 이상 물고기는 살아 있음
			printf("물고기가 아직 살아있어요!\n");
		}
		prevElasedTime = totalElapsedTime;
